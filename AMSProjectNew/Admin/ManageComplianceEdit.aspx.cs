...
{
    public partial class ManageComplianceEdit : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    FillComplianceDetails();
                }
            }
        }

        private void FillComplianceDetails()
        {
            ComplianceController complianceController = new ComplianceController();
            DataSet ds = new DataSet();
            try
            {
                ds = complianceController.ComplianceSelectAll(Convert.ToInt64(Request.QueryString["Id"]), 0);
        ...
        protected void btnCheckAvailabilty_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                Id = usersController.UsersLoginCreate(txtUsername.Text.Trim(), txtPassword.Text.Trim(), "Compliance", "CHECKUSERNAME");
                if (Id > 0)
                {
        ...
        {
            Response.Redirect("~/Admin/ManageCompliance.aspx", false);
        }
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            {                
                ...
                if (Request.QueryString["Id"] != null && Convert.ToString(Request.QueryString["Id"]) != "")
                {
                    ...
                    Id = complianceController.ComplianceEdit(Id, Id, txtEmailAddress.Text.Trim(),
                    ...
                    {
                        lblError.Text = "Compliance details does not submitted successfully. Please try again later!";
                        return;
                    }
                }
                else
                {
                    ...
                    if (Id > 0)
                    {
                        Id = complianceController.ComplianceEdit(0, Id, txtEmailAddress.Text.Trim(),
                            txtFirstName.Text.Trim(), txtLastName.Text.Trim(), txtAddress.Text.Trim(),
                            txtSuburb.Text.Trim(), txtState.Text.Trim(), txtPostcode.Text.Trim(), txtPhone1.Text.Trim(),
                            txtPhone2.Text.Trim(), txtOtherDetails.Text.Trim(), txtFax.Text.Trim(),
                            Convert.ToInt64(Session["UserId"]), Convert.ToInt64(Session["UserId"]), 
        ...
        private long CreateLogin()
        {
            ...
            {                
                ...
                {
                    Id = usersController.UsersLoginCreate(txtUsername.Text.Trim(), txtPassword.Text.Trim(), "Compliance", "CREATE");
                }
            }
            catch (Exception ex)
            {
