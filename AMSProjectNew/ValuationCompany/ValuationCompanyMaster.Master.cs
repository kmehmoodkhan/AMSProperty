...
{
    public partial class ValuationCompanyMaster : System.Web.UI.MasterPage
    {
            ...
            if (Session["UserId"] != null && Convert.ToString(Session["UserId"]) != "")
            {
                if (Session["FullName"] != null && Convert.ToString(Session["FullName"]) != "")
                {
                    ...
                    if (Convert.ToString(Session["LastLoggedOn"]) != "")
                        lblLastloggedOn.Text = Convert.ToDateTime(Session["LastLoggedOn"]).ToString("dd/MM/yyyy hh:mm:ss");
                    else
